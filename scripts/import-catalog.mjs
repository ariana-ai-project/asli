/* ============================================================
   ...
   مرورگر، یک فایل SQL می‌سازد که یک‌جا اجرا می‌شود:
     ...
     node scripts/import-catalog.mjs <پوشهٔ چهار فایل> <خروجی.sql>
   ...
   است (عیناً در vm اجرا می‌شود) و ستون‌ها و DDL همان worker/catalog.js. هیچ
   منطقِ دومی این‌جا نیست که بتواند واگرا شود.
...
import { TABLES, CATALOG_DDL } from "../worker/catalog.js";

const HERE = dirname(fileURLToPath(import.meta.url));
const FRONT = resolve(HERE, "../frontend/tamin-poshtibani");
const [dir, out] = process.argv.slice(2);
if (!dir || !out) { console.error("کاربرد: node scripts/import-catalog.mjs <پوشهٔ چهار فایل> <خروجی.sql>"); process.exit(2); }
...
for (const [t, rows] of Object.entries(data.tables)) {
  ...
  const head = `INSERT OR IGNORE INTO ${t} (${cols.join(",")}) VALUES `;
  let cur = [], size = 0;
...
sql.push(`INSERT INTO settings (key,value,updated_at) VALUES ('catalog',${lit(JSON.stringify({ ...data.meta, fp: data.fp }))},${t}) ON CONFLICT(key) DO UPDATE SET value=excluded.value, updated_at=excluded.updated_at;`);
...
console.log(`نوشتن در D1: ≈ ${writes.toLocaleString("en-US")} ردیف (+ ۳ ردیف settings و hist_imports)`);
for (const [k, rows] of Object.entries(data.tables)) console.log(`  ${k.padEnd(16)} ${String(rows.length).padStart(6)}`);
