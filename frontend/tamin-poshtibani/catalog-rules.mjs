...
export const MATERIALS = {
  ...
  "گالوانیزه": ["گالوانیزه", "گالوانیزه گرم", "گالوانیزه سرد", "گالوانیز"],
  "گالوالوم": ["گالوالوم"],
...
export const UNITS = {
  ...
  "کیلوکالری بر ساعت": { dim: "توان گرمایی", k: 1, forms: ["کیلوکالری", "کیلو کالری"] },
  "BTU/h": { dim: "توان گرمایی", k: 0.252, forms: ["btu/h", "btu"] },
...
export const QUANT = {
  ...
  "حجم": ["حجم"], "وزن": ["جرم"], "توان": ["توان", "توان ظاهری", "توان گرمایی"], "ولتاژ": ["ولتاژ"],
 ...
 *   d — پیش‌فرض ثابت؛ c — نشانهٔ واحد یا واژه در عنوان، به ترتیب؛ f — وقتی هیچ نشانه‌ای نخورد.
...
export function impliedMaterial(rule, title, units) {
  ...
  for (const [cue, v, m] of rule.c || []) {
    if (cue === "u" && units.has(v)) return m;
    if (cue === "w" && t.includes(` ${keyOf(v)} `)) return m;
  }
  return rule.f || null;
}
