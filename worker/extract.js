...
export const SCHEMA = {
  ...
  properties: {
    ...
    vat_status: {
      type: ["string", "null"],
      enum: ["دارد", "ندارد", null],
      description: "آیا این معامله ارزش افزوده دارد؟ «دارد» اگر سند هر جا (چه در قلم، چه ته فاکتور) ارزش افزوده آورده یا گفته اضافه می‌شود؛ «ندارد» اگر صریح گفته معاف است یا ارزش افزوده ندارد؛ اگر هیچ اشاره‌ای نشده null.",
    },
    vat_included: {
      type: ["boolean", "null"],
...
اصل هشتم — ارزش افزوده جداگانه است.
در جدول کمیسیون، قیمتِ هر ردیف **بدون** ارزش افزوده می‌نشیند و ارزش افزوده یک سطر جدا در انتهای جدول دارد.
پس unit_price را همان‌طور که در سند نوشته شده بنویس و در vat_included صادقانه بگو آن عدد ارزش افزوده را در خود دارد یا نه:
• اگر فاکتور ارزش افزوده را ته صفحه جداگانه اضافه کرده (حالت معمول) → vat_included=false
• اگر ستون قیمت یا مبلغ کلِ همان ردیف ارزش افزوده را در خود دارد → vat_included=true
خودت هیچ عددی را تقسیم یا کم نکن؛ فقط درست بگو کدام حالت است. کم‌کردنش کار سامانه است.
vat_status هم بگو معامله اصلاً ارزش افزوده دارد یا نه — این با vat_included فرق دارد: فاکتوری که ارزش افزوده را ته
صفحه اضافه کرده، vat_status=«دارد» ولی vat_included=false.

اصل نهم — فهرست‌های ثابتِ جدول.
