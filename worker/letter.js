...
export const LETTER_PROMPT_VERSION = "letter/3.1";
...
/* مخاطب نامه (تصمیم مدیر، شهریور ۱۴۰۵): مدیر امور پشتیبانی؛ نامه از زبان خودِ کارشناس خرید است */
export const LETTER_TO = "جناب آقای کوشاری، مدیر محترم امور پشتیبانی";
 ...
 */
const LETTER_SYSTEM = `تو به جای کارشناس خرید واحد تدارکات و پشتیبانی یک شرکت پیمانکاری ایرانی نامهٔ اداری می‌نویسی — نامه‌ای که
او پیوستِ جدول کمیسیون می‌کند، خودش پایش را امضا می‌کند و مدیر امور پشتیبانی می‌خواند.

**نامه از زبان خودِ کارشناس خرید است.** نویسنده اوست: هرگز «کارشناس خرید» را سوم‌شخص ننویس («کارشناس خرید استعلام گرفت»
غلط است). آنچه او کرده را یا اداریِ مجهول بنویس («استعلام اخذ گردید»، «نمونه ارسال شد») یا اول‌شخص («اینجانب»، «موفق به
اخذ تخفیف شدیم») — همان‌طور که خودش نامه می‌نویسد. مخاطب هم خودِ خواننده است (جنابعالی)، نه شخص سوم.
...
export async function writeLetter(env, { transcript, request, items, quotes, allItems, notes, expert, expertName, company, subjectTitles }) {
  ...
  const context = `شرکت: ${company}\n`
    ...
    + `نویسندهٔ نامه (کارشناس خرید، امضاکننده): ${expert}\n\n`
  ...
  const r = await fetch(AI_URL(env), {
    ...
    body: JSON.stringify({
      ...
      messages: [{ role: "user", content: [{ type: "text", text:
        `<اطلاعات_درخواست>\n${context}\n</اطلاعات_درخواست>\n\n`
        + `<حرف_کارشناس>\n${transcript}\n</حرف_کارشناس>\n\n`
        + `بدنهٔ نامه و جملهٔ پایانی را از زبان خودِ کارشناس خرید و به سبک نامه‌های واحد بنویس: یک تا سه بند، بند اول با «احتراماً به استحضار می‌رساند»، بی «کارشناس خرید» در سوم‌شخص. عددها را ننویس؛ فقط جای‌خالی‌های بالا را عیناً بگذار.` }] }],
    }),
  });
